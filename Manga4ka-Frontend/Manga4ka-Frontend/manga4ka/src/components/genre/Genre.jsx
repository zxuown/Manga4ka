    ...
    return (
        <>
            <div className="row justify-content-center">
                {
                    user !== undefined && Array.isArray(user.roles) && user.roles.some(x => x == "Admin") ? <Link to={`/genres/create`} className="btn btn-success">+ Create genre</Link>
                        : ""
                }
                {genres.map((genre, index) => (
                    <div key={index} className={`card mb-3 m-3 col-md-2  ${darkMode ? 'bg-dark text-white border-secondary' : 'bg-light text-dark shadow-sm'}`}>
