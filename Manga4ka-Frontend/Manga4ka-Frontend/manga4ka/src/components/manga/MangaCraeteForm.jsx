...
function MangaCreateForm() {
    ...
    const handleSubmit = (e) => {
        ...
        const fetchCreateManga = async () => {
            try {
                const response = await MangaService.CreateManga(manga, localStorage.getItem('token'))
                if (response.status === 200) {
                    manga.id = response.data
                    const mangaGenres = []
                    ...
                    await MangaService.CreateMangaGenres(mangaGenres, localStorage.getItem('token'))
                    setManga(emptyManga)
                    Swal.fire({
                        title: 'Success!',
