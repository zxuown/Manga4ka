...
function MangaEditForm() {
    ...
    const [newMangaGenresId, setNewMangaGenresId] = useState([])
            ...
            try {
                ...
                if (status === 200) {
                    ...
                    if (mangaGenres.length > 0) {
                        await MangaService.DeleteMangaGenres(mangaGenres, localStorage.getItem('token'))
                    }
                    const newMangaGenres = []
                    genres.filter(genre => newMangaGenresId.includes(genre.id.toString()))
                        .forEach(genre => {
                            newMangaGenres.push({
                                mangaId: manga.id,
                                genreId: genre.id,
                    ...
                    await MangaService.CreateMangaGenres(newMangaGenres, localStorage.getItem('token'))
                    setMangaGenres(newMangaGenres)
                    Swal.fire({
                        title: 'Success!',
    ...
    const handleGenreChange = (e) => {
        ...
        setNewMangaGenresId(selectedGenres)
    }
    ...
    useEffect(() => {
        ...
        const fetchAllMangaGenresByMangaId = async () => {
            try {
                const data = await MangaService.GetMangaGenresByMangaId(mangaId)
                console.log(data)
                setMangaGenres(data)
            } catch (e) {
                console.error("Error loading all user manga genres by manga id", e)
            }
        }
