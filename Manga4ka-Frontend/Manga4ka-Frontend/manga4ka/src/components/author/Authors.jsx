            ...
            <div className='row justify-content-center'>
                {
                    user !== undefined && Array.isArray(user.roles) && user.roles.some(x => x == "Admin") ?
                        <Link to={`/authors/create`} className="btn btn-success">+ Create author</Link>
                        : ""
                }
                {authors.map((author, index) => (
                    <div key={index} className={`card mb-3 m-3 col-md-3 ${darkMode ? 'bg-dark text-white border-secondary' : 'bg-light text-dark shadow-sm'}`} style={{ maxWidth: 400 }}>
