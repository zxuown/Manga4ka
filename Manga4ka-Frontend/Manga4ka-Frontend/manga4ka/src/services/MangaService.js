...
class MangaService {
    ...
    static async GetMangaGenres() {
        try {
            const response = await axios.get(`${API}/mangaGenres`)
            return response.data
        } catch (error) {
            console.error('Error fetching all manga genres', error)
            throw error
        }
    }

    static async GetMangaGenresByMangaId(mangaId) {
        try {
            const response = await axios.get(`${API}/mangaGenres/${mangaId}`)
            return response.data
        } catch (error) {
            console.error('Error fetching all manga genres by manga id', error)
            throw error
        }
    }

    static async CreateMangaGenres(mangaGenres, token) {
        try {
            const response = await axios.post(`${API}/mangaGenres`, JSON.stringify(mangaGenres), {
                headers: {
            ...
            console.error('Error creating manga genres', error)
            throw error
        }
    }

    static async DeleteMangaGenres(mangaGenres, token) {
        try {
            const response = await axios.delete(`${API}/mangaGenres`, {
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
                },
                data: JSON.stringify(mangaGenres)
            })
            return response.status
        } catch (error) {
            console.error('Error deleting manga genres', error)
            throw error
        }
    }
